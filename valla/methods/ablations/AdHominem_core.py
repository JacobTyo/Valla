...
class AdHominem():
    ...
    """
    def __init__(self, hyper_parameters, E_w_init, theta_init=None):
        ...
        if theta_init is None:
            ...
            self.placeholders, self.thetas_E = self.restored_initialize_placeholders(E_w_init)
    ...
    ################################################
    def restored_initialize_placeholders(self, theta_E):
    ...
    def initialize_parameters(self, theta_init=None):
        ...
        if theta_init is None:
            ...
            with tf.variable_scope('theta_cnn'):
                theta['cnn'] = self.pt_initialize_cnn(theta_init['cnn'], True)

            with tf.variable_scope('theta_lstm_ws_forward'):
                theta['lstm_ws_forward'] = self.pt_initialize_lstm(theta_init['lstm_ws_forward'], True)

            with tf.variable_scope('theta_lstm_ws_backward'):
                theta['lstm_ws_backward'] = self.pt_initialize_lstm(theta_init['lstm_ws_backward'], True)

            with tf.variable_scope('theta_lstm_sd_forward'):
                theta['lstm_sd_forward'] = self.pt_initialize_lstm(theta_init['lstm_sd_forward'], True)

            with tf.variable_scope('theta_lstm_sd_backward'):
                theta['lstm_sd_backward'] = self.pt_initialize_lstm(theta_init['lstm_sd_backward'], True)

            with tf.variable_scope('theta_att_ws'):
                theta['att_ws'] = self.pt_initialize_att(theta_init['att_ws'], True)

            with tf.variable_scope('theta_att_sd'):
                theta['att_sd'] = self.pt_initialize_att(theta_init['att_sd'], True)

            with tf.variable_scope('theta_metric'):
                theta['metric'] = self.pt_initialize_mlp(theta_init['metric'], True)

            return theta

    def pt_initialize_mlp(self, theta_init, retrain):
    ...
    def pt_initialize_cnn(self, theta_init, retrain):
        theta = {'W': tf.Variable(theta_init['W'],
                                  trainable=retrain,
                                  dtype=tf.float32,
                                  name='W',
    ...
    def pt_initialize_att(self, theta_init, retrain):
        theta = {'W': tf.Variable(theta_init['W'],
                                  trainable=retrain,
                                  dtype=tf.float32,
                                  name='W',
    ...
    def pt_initialize_lstm(self, theta_init, retrain):
    ...
    def grid_search(pred, labels):
        pred.reshape(-1)
        labels.reshape(-1)
        L = list(np.linspace(np.min(pred), np.max(pred), 100))
        scores = []
        scores_o = []
        print(f'grid searching, theashold range, min: {np.min(pred)}, max: {np.max(pred)}')
        for l in L:
            pred_l = pred.copy()
            pred_l[pred < l] = 0
            pred_l[pred >= l] = 1
            s = av_metrics(labels, pred_l, pred)
            # s = evaluate_all(pred_y=pred_l, true_y=labels)
            scores.append(s)
            scores_o.append(s['auc'])
        ...
        for epoch in range(epochs):
            ...
            for i in range(num_batches_tr):
                ...
                if B > 0:
                    ...
                    wandb.log({
                        'epoch': epoch,
                        'batch': round(100 * (i + 1) / num_batches_tr, 2),
                        'loss': float(np.mean(loss)),
                        'train/accuracy': (100 * (TP + TN) / (TP + FP + TN + FN), 2),
                        'train/loss': curr_loss,
                        'batch/accuracy': round(100 * curr_acc, 2),
                        'lr': float(lr[epoch])
                    })
            ...
            res = {'epoch': epoch,
                   'loss': float(np.mean(loss)),
                   'train/accuracy': round(100 * acc_tr, 4),
                   'test/accuracy': round(100 * acc_te, 4),
                   'threshold': th}
            res.update(scores)
            wandb.log(res)

            if self.hyper_parameters['save_model_path'] is not None:
                self.store_parameters(epoch, dire_results=self.hyper_parameters['save_model_path'])
