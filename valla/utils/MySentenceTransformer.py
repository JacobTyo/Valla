            ...
            for _ in trange(steps_per_epoch, desc="Iteration", smoothing=0.05):
                for train_idx in range(num_train_objectives):
                    ...
                    if use_amp:
                        ...
                        loss_value, sim_stats = loss_model(features, labels)
                        loss_value.backward()
                        torch.nn.utils.clip_grad_norm_(loss_model.parameters(), max_grad_norm)
                        optimizer.step()

                    optimizer.zero_grad()
                    training_loss_tracking.append(loss_value.item())
                    training_res_tracking.append(sim_stats)
                    if time.time() - last_log_time > train_loss_log_interval:
                        log_me_idx = np.random.randint(0, len(training_res_tracking))
                        step = epoch * steps_per_epoch + training_steps
                        res = {
                            'training_loss_mean': np.mean(training_loss_tracking),
                            'training_loss_max': np.max(training_loss_tracking),
                            'training_loss_min': np.min(training_loss_tracking),
                            'training_loss_median': np.median(training_loss_tracking),
                            'training_loss_std': np.std(training_loss_tracking)
                        }
                        for k in training_res_tracking[0].keys():
                            vals = [x[k] for x in training_res_tracking]
                            res.update({
                                k + '_mean': np.mean(vals),
                                k + '_sing': vals[log_me_idx]
                            })
